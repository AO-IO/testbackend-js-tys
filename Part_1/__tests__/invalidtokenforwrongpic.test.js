...
const viewImage = await request(app).get('/image/'+requestBody.main_image)
  ...
  const ViewFirstImageWithtoken2 = await request(app).get('/image/'+requestBody.main_image)
  .set('Authorization', `Bearer ${token2}`).expect(403)

  expect(ViewFirstImageWithtoken2.body).toHaveProperty('message','Invalid token, are you sure its valid?')
